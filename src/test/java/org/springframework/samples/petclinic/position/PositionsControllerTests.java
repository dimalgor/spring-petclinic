...
class PositionsControllerTests {
	...
	void testInitCreationForm() throws Exception {
		mockMvc.perform(get("/positions/new")).andExpect(status().isOk()).andExpect(model().attributeExists("position"))
	...
	void testInitFindForm() throws Exception {
		mockMvc.perform(get("/positions/find")).andExpect(status().isOk()).andExpect(model().attributeExists("position"))
				.andExpect(view().name("positions/findPositions"));
	}

	@Test
	void testProcessFindFormSuccess() throws Exception {
		given(this.positions.findByPosition("")).willReturn(Lists.newArrayList(george, new Position()));
		mockMvc.perform(get("/positions")).andExpect(status().isOk()).andExpect(view().name("positions/positionsList"));
	}
	...
	@Test
	void testProcessUpdatePositionFormHasErrors() throws Exception {
		mockMvc.perform(post("/positions/{positionId}/edit", TEST_POSITION_ID).param("area", "Joe")
	...
	void testShowPosition() throws Exception {
		mockMvc.perform(get("/positions/{position}", TEST_POSITION_ID)).andExpect(status().isOk())
				...
				}))).andExpect(view().name("positions/positionDetails"));
	}
